...
- **数据源（全部真实第三方服务）**：
  - SerpApi — Amazon 竞品/市场数据（无需 Amazon 官方 API）
  - 万邦 OneBound — 1688 货源搜索/详情（无需 1688 官方 API）
  - exchangerate-api / open.er-api — 实时汇率
- **LLM**：OpenAI / Anthropic / 通义（OpenAI 兼容模式）可配置切换
...
| 变量 | 服务 | 用途 |
|---|---|---|
| `OPENAI_API_KEY` 等 | OpenAI/Anthropic/通义 | 分析与内容生成 |
| `SERPAPI_API_KEY` | serpapi.com | Amazon 竞品数据 |
| `ONEBOUND_API_KEY/SECRET` | open.onebound.cn | 1688 货源数据 |
